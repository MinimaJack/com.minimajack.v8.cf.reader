...
public class SimpleFileContainerWriter extends ContainerWriter {
	...
	public SimpleFileContainerWriter(
			File path, boolean packed) {
	...
	public void getDataToWrite() {
		...
		if(currentFile.isDirectory()){
			SimpleFileContainerWriter fscw = new SimpleFileContainerWriter(
					currentFile,
					false);
			fscw.writeAllData();
		...
		if(this.packed){
			...
			DeflaterOutputStream dataStream = new DeflaterOutputStream(baos
					, new Deflater(6, true));
			try {
